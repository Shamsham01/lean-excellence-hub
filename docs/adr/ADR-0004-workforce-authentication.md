...
Accepted and implemented for workforce login, resolver, and M1 administrator
provisioning. Microsoft OAuth remains deferred.
...
- administrator-initiated provisioning with system-generated temporary passwords
  (see [ADR-0014](ADR-0014-workforce-provisioning-and-notification-foundation.md));
...
- M1 administrator provisioning returns a temporary password once to authorised
  Organisation Owners and Organisation Administrators; it is never stored or
  retrievable afterwards. Employees own their permanent password after first
  login.
...
defines administrator provisioning, temporary password handling, and signup-hook
interaction for privileged workforce account creation.
