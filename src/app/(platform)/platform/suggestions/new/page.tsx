...
export default async function NewSuggestionPage() {
    ...
    if (versionsError) {
      ...
      if (programmesError) {
        ...
        programmesForForm = programmeVersions.map((row) => ({
          id: row.id,
          programme_name: programmeNameById.get(row.programme_id) ?? "Programme",
        }));
      }
    }
  }
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col gap-6" data-testid="new-suggestion-page">
      <PageHeader
      ...
      <NewSuggestionForm programmeVersions={programmesForForm} categories={categories ?? []} />
    </div>
  );
}
