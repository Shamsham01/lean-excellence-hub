...
export function RecognitionHistory({ awards, canManage }: RecognitionHistoryProps) {
  ...
  return (
    <Card data-testid="recognition-history">
      ...
      <CardContent className="flex flex-col gap-3">
        {awards.map((award) => (
          <div key={award.id} className="rounded-md border border-border p-3 text-sm">
                ...
                {revokingId === award.id ? (
                  <>
                    <label className="flex flex-col gap-1">
                      <Label htmlFor={`revoke-reason-${award.id}`}>Revocation reason</Label>
                      <Textarea
