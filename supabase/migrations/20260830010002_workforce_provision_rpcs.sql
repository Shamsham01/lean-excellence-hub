...
as $$
  select not exists (
    ...
    where workforce_alias.organisation_id = target_organisation_id
      and workforce_alias.canonical_alias = target_canonical_alias
      and workforce_alias.status = 'active'
  )
  and not exists (
    select 1
    from public.workforce_provision_intents intent_row
    where intent_row.organisation_id = target_organisation_id
      and intent_row.target_canonical_alias = target_canonical_alias
...
begin
  ...
  update public.workforce_provision_intents
  set status = case
        when created_auth_user_id is not null then 'needs_platform_remediation'
        else 'failed'
      end,
