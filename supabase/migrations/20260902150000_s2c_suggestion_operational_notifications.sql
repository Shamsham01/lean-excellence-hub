-- S2c: Suggestion operational notification delivery context and authorization.
...
create or replace function private.membership_is_current_active_suggestion_reviewer(
  target_organisation_id uuid,
  target_suggestion_id uuid,
  target_membership_id uuid
)
...
begin
  ...
  if delivery_row.notification_kind = 'workforce.job_function_assigned' then
    ...
    where suggestion_table.organisation_id = target_organisation_id
      and suggestion_table.id = event_row.resource_record_id
    limit 1;

    if suggestion_row.id is not null then
      resolved_title := coalesce(
        nullif(btrim(suggestion_row.title), ''),
        nullif(btrim(suggestion_row.suggestion_number), ''),
        'Suggestion'
      );

      if delivery_row.notification_kind = 'suggestions.reviewer_assigned' then
        resolved_detail := 'Assigned to you for review';
        resolved_link_path :=
          '/platform/suggestions/review?queue=mine&suggestionId='
          || suggestion_row.id::text;
      elsif delivery_row.notification_kind = 'suggestions.reviewer_reassigned' then
        resolved_detail := 'Reassigned to you for review';
        resolved_link_path :=
          '/platform/suggestions/review?queue=mine&suggestionId='
          || suggestion_row.id::text;
      elsif delivery_row.notification_kind = 'suggestions.review_started' then
        resolved_detail := 'Your suggestion is now under review';
        resolved_link_path := '/platform/suggestions/' || suggestion_row.id::text;
      elsif delivery_row.notification_kind = 'suggestions.approved' then
        resolved_detail := 'Your suggestion was approved';
        resolved_link_path := '/platform/suggestions/' || suggestion_row.id::text;
      elsif delivery_row.notification_kind = 'suggestions.declined' then
        resolved_detail := 'Your suggestion was declined';
        resolved_link_path := '/platform/suggestions/' || suggestion_row.id::text;
      elsif delivery_row.notification_kind = 'suggestions.parked' then
        resolved_detail :=
  ...
  if resolution_status = 'deliverable'
    and delivery_row.notification_kind in (
      'suggestions.reviewer_assigned',
      'suggestions.reviewer_reassigned'
    )
    and event_row.resource_record_id is not null then
    if not private.membership_is_current_active_suggestion_reviewer(
      target_organisation_id,
  ...
  elsif resolution_status = 'deliverable'
    and delivery_row.notification_kind in (
      'suggestions.review_started',
      'suggestions.approved',
      'suggestions.declined',
    ...
    if suggestion_row.id is null
      or suggestion_row.author_membership_id is distinct from delivery_row.recipient_membership_id
...
alter function private.membership_is_current_active_suggestion_reviewer(uuid, uuid, uuid)
  owner to lean_hub_private_owner;

revoke all on function private.membership_can_read_improvement_suggestion(uuid, uuid, uuid)
  from public, anon, authenticated;
revoke all on function private.membership_is_current_active_suggestion_reviewer(uuid, uuid, uuid)
  from public, anon, authenticated;

grant execute on function private.membership_can_read_improvement_suggestion(uuid, uuid, uuid)
  to postgres, service_role;
grant execute on function private.membership_is_current_active_suggestion_reviewer(uuid, uuid, uuid)
  to postgres, service_role;
