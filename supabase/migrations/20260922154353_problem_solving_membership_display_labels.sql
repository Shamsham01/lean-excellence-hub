...
-- broadening organisation_memberships / profiles RLS or using service_role.

create or replace function private.is_usable_public_display_name(
  target_value text
)
returns boolean
language sql
...
as $$
  select
    ...
    and btrim(target_value) !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    and btrim(target_value) !~* '^[0-9a-f]{8}$'
    and position('@' in btrim(target_value)) = 0
$$;

create or replace function private.membership_public_display_name(
...
as $$
  select
    case
      when private.is_usable_public_display_name(membership_row.display_name)
        then btrim(membership_row.display_name)
      when private.is_usable_public_display_name(profile_row.display_name)
        then btrim(profile_row.display_name)
      else null
    end
...
as $$
  select coalesce(
    jsonb_object_agg(labeled.membership_id::text, labeled.label),
    '{}'::jsonb
  )
  from (
    select
...
declare
  ...
  source_links jsonb;
  current_stage jsonb;
  membership_display_names jsonb;
begin
  ...
  return jsonb_build_object(
    ...
    'owner_display_name', private.membership_public_display_name(
      org_id,
      case_row.owner_membership_id
    ),
    'facilitator_membership_id', case_row.facilitator_membership_id,
    'facilitator_display_name', private.membership_public_display_name(
      org_id,
      case_row.facilitator_membership_id
    ),
    ...
    'membership_display_names', membership_display_names
  );
end;
$$;
