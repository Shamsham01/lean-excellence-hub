...
test.describe("S3a suggestions portfolio", () => {
  ...
  test("pagination preserves filters and supports next page", async ({
    ...
    await expect(nextPage).toHaveAttribute("href", /q=changeover/);
    await expect(nextPage).toHaveAttribute("href", /status=implemented/);
    await expect(nextPage).toHaveAttribute("href", /sort=oldest/);
    await expect(nextPage).toHaveAttribute("href", /pageSize=50/);
    ...
    await expect(previousPage).toHaveAttribute("href", /pageSize=50/);
